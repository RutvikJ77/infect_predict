"""
Inferencing functions for the model.
"""
...
import pandas as pd
import numpy as np
from utils.preprocess import process_df, series_to_supervised, oof_idx, scale_data
...
def infer_all(df, nometa=True, meta=True, oof=False, future_pred=100, save_name='predictions.csv'):
    ...
    if oof:
        ...
        plt.show()
        logging.info(f"{te_idx[0]} mse {mean_squared_error(df['total_cases'].values[window_size:], preds)}")
